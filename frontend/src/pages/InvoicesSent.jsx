...
function InvoicesSent() {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/invoices/?user_id=${user?.sub}&status_filter=sent`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
                        ...
                        <div>
                          ...
                          <p style={{ margin: '4px 0 0 0', color: '#666', fontSize: '14px' }}>
                            Status: <span style={{ 
                              ...
                              fontSize: '12px',
                              fontWeight: '600'
                            }}>{selectedInvoice.invoice.status?.toUpperCase() || 'DRAFT'}</span>
                          </p>
                        </div>
